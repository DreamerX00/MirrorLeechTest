...
async def mirror(client, message):
    ...
    is_authorized, auth_message = await check_user_authorization(user_id, "mirror")
    if not is_authorized:
        auth_button = get_auth_button(user_id)
        await sendMessage(message, "🔒 **Authorization Required**\n\nYou need to verify your subscription to use this command.", auth_button)
        return
...
async def qb_mirror(client, message):
    ...
    is_authorized, auth_message = await check_user_authorization(user_id, "qbmirror")
    if not is_authorized:
        auth_button = get_auth_button(user_id)
        await sendMessage(message, "🔒 **Authorization Required**\n\nYou need to verify your subscription to use this command.", auth_button)
        return
...
async def leech(client, message):
    ...
    is_authorized, auth_message = await check_user_authorization(user_id, "leech")
    if not is_authorized:
        auth_button = get_auth_button(user_id)
        await sendMessage(message, "🔒 **Authorization Required**\n\nYou need to verify your subscription to use this command.", auth_button)
        return
...
async def qb_leech(client, message):
    ...
    is_authorized, auth_message = await check_user_authorization(user_id, "qbleech")
    if not is_authorized:
        auth_button = get_auth_button(user_id)
        await sendMessage(message, "🔒 **Authorization Required**\n\nYou need to verify your subscription to use this command.", auth_button)
        return
