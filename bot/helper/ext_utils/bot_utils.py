...
async def check_user_authorization(user_id: int, command: str = None):
    """Check if user is authorized via auth_bot"""
    try:
        # Check if auth bot integration is enabled
        if not config_dict.get("AUTH_BOT_ENABLED", False):
            return True, None
            
        from auth_bot.utils.integration import intercept_command
        is_authorized, message = await intercept_command(user_id, command or "")
        return is_authorized, message
    except ImportError:
        # Auth bot not available, allow access
        return True, None
    except Exception as e:
        LOGGER.error(f"Auth check error: {e}")
        return True, None  # Fail-safe: allow access

def get_auth_button(user_id: int):
    """Generate auth bot button for unauthorized users"""
    try:
        # Check if auth bot integration is enabled
        if not config_dict.get("AUTH_BOT_ENABLED", False):
            return None
        ...
        if not auth_bot_username:
            return None
            
        from auth_bot.utils.token_manager import generate_token
        
        # Generate temporary verification token
        token = generate_token(user_id, 0)  # 0 days = temporary token
        auth_url = f"https://t.me/{auth_bot_username}?start=verify_{token}"
        
        from bot.helper.telegram_helper.button_build import ButtonMaker
        buttons = ButtonMaker()
        buttons.ubutton("🔐 Get Authorization", auth_url)
        return buttons.build_menu(1)
    except Exception as e:
        LOGGER.error(f"Error generating auth button: {e}")
        return None
