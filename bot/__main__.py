...
async def start(client, message):
    ...
    # Check for auth_bot verification token
    if len(message.command) > 1 and message.command[1].startswith('verify_'):
        try:
            from auth_bot.utils.integration import validate_verification_token
            token = message.command[1].replace('verify_', '')
            is_valid, token_data = await validate_verification_token(token)
            
            if is_valid:
                await sendMessage(message, "✅ **Verification Successful!**\n\nYou now have access to all bot features.", reply_markup)
            else:
                await sendMessage(message, "❌ **Invalid or Expired Token**\n\nPlease request a new verification link.", reply_markup)
            return
